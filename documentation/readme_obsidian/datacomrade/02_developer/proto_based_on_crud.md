...
Второй шаг цепочки: по готовому `query.sql` строим `.proto`, точно повторяющий набор CRUD-операций.

- Инструкция: `documentation/dev_instructions/crud/proto_based_on_crud.md`
- Вход: `query.sql` (список операций) и `schema.sql` (типы и состав полей)
- Выход: `shared/proto/<домен>/v1/<домен>.proto`
...
При 15+ таблицах это около 270 сообщений — руками не набирать, будут опечатки. Пишется одноразовый скрипт: список таблиц с полями → генерация текста → запись `.proto`. Так правки схемы применяются перегенерацией, а нумерация полей и имена сообщений гарантированно согласованы.
