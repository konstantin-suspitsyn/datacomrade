# -*- coding: utf-8 -*-
"""Генерация tables.proto из schema.sql/query.sql домена tables_model.

Реализует правила из documentation/dev_instructions/crud/proto_based_on_crud.md:
8 стандартных RPC на таблицу плюс FK-выборки, с спецмаппингом
"<col>_id = (SELECT u.id FROM dc.\"user\" u WHERE u.external_id = $N)"
в proto-поле "<col>_external_id" (см. EXTERNAL_ID_SUFFIX в resolve.py).

Запуск:  python deploy/generators/proto_gen_tables.py
"""
import re

SCHEMA_PATH = r"C:\Users\const\working\datacomrade\datacatalogue\db\sqlc\tables_model\schema.sql"
QUERY_PATH = r"C:\Users\const\working\datacomrade\datacatalogue\db\sqlc\tables_model\query.sql"
OUT_PATH = r"C:\Users\const\working\datacomrade\shared\proto\tables\v1\tables.proto"
...
ENTITY_NAMES = {
    ...
    'database_calculation': 'DatabaseCalculation',
    'database_cat': 'DatabaseCat',
    'database_type': 'DatabaseType',
    'domain_cat': 'DomainCat',
    'following_calculation': 'FollowingCalculation',
    'group_levels': 'GroupLevel',
    'has_to_group': 'HasToGroup',
    'host': 'Host',
    'schema_cat': 'SchemaCat',
    'table_cat': 'TableCat',
...
def parse_schema(path):
    ...
    tables = {}
    for m in re.finditer(r'CREATE TABLE dc\.(\w+) \((.*?)\n\);', text, re.DOTALL | re.IGNORECASE):
        table = m.group(1)
        body = m.group(2)
        cols = []
...
def value_to_field(col_name, value, table_cols):
    """Return (field_name, proto_type, is_message) or None if literal (no param)."""
    value = value.strip()
...
def parse_queries(path):
    text = open(path, encoding='utf-8').read()
    # split into table sections by divider comments
    ...
    for idx, (table, queries) in enumerate(sections):
        ...
        for name, qtype, sql in queries:
            if name == f'Get{entity}ById' or name == f'GetDeleted{entity}ById':
                ...
                # Get<E>sBy<Fk> or Get<E>By<Column>
...
option go_package = "github.com/konstantin-suspitsyn/datacomrade/shared/pkg/proto/tables/v1;tablesv1";
